...
export const ItemDetails = () => {
  ...
  const handleCopy = (id) => {
    ...
    const contentToCopy = `
      ...
      Shelf ID: ${itemToCopy.shelfId}
      Warehouse: ${itemToCopy.warehouse.join(", ")}
      Stock: ${itemToCopy.stock}
      Expiry Date: ${itemToCopy.date}
      SKU: ${itemToCopy.sku}
      Barcode: ${itemToCopy.barcode}
      Price: $${itemToCopy.price}
      Description: ${itemToCopy.description}
      Reorder Level: ${itemToCopy.reorderLevel}
    `;

    navigator.clipboard
      .writeText(contentToCopy)
      .then(() => {
        console.log("Item details copied to clipboard:", contentToCopy);
        setCopied(true);
        setTimeout(() => {
          setCopied(false);
        }, 2000);
        ...
        {isEditing ? (
          ...
          <Card className="w-full">
            <CardHeader>
              ...
              <CardDescription>Details of the selected item.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid w-full items-center gap-4 md:grid-cols-2">
                ...
                <div className="flex flex-col space-y-1.5">
                  <Label>Barcode</Label>
                  <Input value={item.barcode} readOnly />
                </div>
                <div className="flex flex-col space-y-1.5">
                  <Label>Stock</Label>
