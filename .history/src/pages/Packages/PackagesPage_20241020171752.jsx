    ...
    return packages
      ...
      .map((pkg) => (
        <Card key={pkg.id} className="mb-4">
          <CardHeader className="flex items-center justify-between w-full space-x-2">
  {/* Ensure title takes only as much space as needed */}
  <div className="flex-1">
    <CardTitle className="truncate">{pkg.packageNumber}</CardTitle>
  </div>

  {/* Popover Button aligned to the right */}
  <Popover>
    ...
    <PopoverContent className="w-48">
      {status === "not shipped" && (
        <Button variant="ghost" onClick={() => handleShipManually(pkg)}>
          Ship Manually
        </Button>
      )}
      {status === "shipped" && (
        <Button variant="ghost" onClick={() => handleMarkAsDelivered(pkg)}>
          Mark as Delivered
        </Button>
      )}
    </PopoverContent>
