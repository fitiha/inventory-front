...
const PackagesPage = () => {
  ...
  const renderPackages = (status) => {
    return packages
      ...
      .map((pkg) => (
        <Card key={pkg.id} className="mb-4 p-2">
          <CardHeader className="flex flex-row items-center justify-between m-0 p-0">
            <CardTitle className="w-fit mt-2 pl-4">{pkg.customer}</CardTitle>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" className="h-8 w-8 p-0 hover:bg-primary pr-4">
                  <MoreVertical className="h-4 w-4 " />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="bg-yellow-500 w-fit p-0">
                {status === "not shipped" && (
                  <Button
                    variant="ghost"
          ...
          <CardDescription className="px-4 grid grid-cols-2 gap-x-8">
            <p className="text-primary font-semibold">{pkg.packageNumber}</p>
            {pkg.shippingCharges && <p className="font-bold"> ${pkg.shippingCharges}</p>}
